...
- [x] `[react]` `npm run new:component <ui|app|domains/<vertical>> <Name>` scaffold: component file from the AGENTS.md template, a standalone showcase section file (imported directly by `showcase.data.tsx` rather than inserted into an existing category aggregator), a test file, a nav entry in `showcase.menu.ts`, and a reminder to run the snapshot. Verified end-to-end (scaffolded, type-checked, ran the generated test, confirmed the page renders on a live server, then cleaned up).
- [x] `[ejs]` Same scaffold, adapted: `npm run new:component <ui|app|domain/<vertical>> <Name>` generates a partial + standalone showcase section (`previewHtml` as a hand-rolled string, matching this codebase's existing convention) + `showcase.data.ts`/`showcase.menu.ts` edits. Does **not** scaffold a `.locals.ts` file — that convention doesn't exist anywhere yet since phase 6 hasn't happened, so there's nothing established to generate against.
