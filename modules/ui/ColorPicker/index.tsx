'use client';
// modules/ui/ColorPicker/index.tsx
//
// ColorPicker — popover-style swatch grid + hex/native picker with an
// optional HEX/RGBA/HSLA/HWB/OKLCH format switcher (M1).
//
// Pixel-identical EJS sibling at modules/ui/ColorPicker/ColorPicker.ejs.
//
// TODO M2: SaturationCanvas + HueSlider + AlphaSlider, palette presets,
//          recent colors (localStorage), native EyeDropper.
// TODO M3: GradientEditor (linear/radial).
// TODO M4: ContrastBadge (WCAG ratio + AA/AAA classifier).
// TODO M5: full keyboard nav inside the SV pad, ARIA live announcements,
//          messages prop for i18n, reduced-motion.
...
import { useColorState, formatRgbaAs } from './hooks/useColorState';
...
/** Legacy hex normalizer kept for the showcase + hex-only input branch. */
...
export function ColorPicker({
  ...
  // Centralised RGBA state — drives the format-switcher InputRow.
  const { rgba, setRgba, syncFromString } = useColorState({
    value: value ?? null,
    format,
  });
  ...
  /** Update RGBA from the InputRow and forward a re-formatted string. */
  const handleRgbaChange = (next: RGBA) => {
    setRgba(next);
    onChange(formatRgbaAs(next, format));
  };

  /** Swap format — emits the same color in the new representation. */
  const handleFormatChange = (next: ColorFormat) => {
    setFormat(next);
    // Re-emit the current color in the new format so the caller sees the
    // same color string they're now visually committed to.
    if (value) onChange(formatRgbaAs(rgba, next));
  };

  // For the legacy `<input type="color">` the browser only accepts #RRGGBB.
  const nativeHex = useMemo(() => {
    const fromValue = normalizeHex(value ?? '');
