...
type MenuState = {
  /** Raw click coordinates (before boundary adjustment) */
  rawX: number;
  rawY: number;
  /** Final coordinates, set after measuring the rendered element */
  adjX: number;
  adjY: number;
  /** False until the first paint+measure cycle completes */
  measured: boolean;
};
...
export function ContextMenu({
  ...
  /* — Measure & adjust position after first render ——————————————————————— */
  ...
  useEffect(() => {
    ...
    const adjX = menu.rawX + width > vw - GAP
      ? Math.max(GAP, menu.rawX - width)
      : menu.rawX;

    const adjY = menu.rawY + height > vh - GAP
      ? Math.max(GAP, menu.rawY - height)
      : menu.rawY;
  ...
  /* — Focus active item button ————————————————————————————————————————————— */

  useEffect(() => {
    if (!menuRef.current || focusedActionIdx < 0) return;
    const btn = menuRef.current.querySelector<HTMLButtonElement>(
